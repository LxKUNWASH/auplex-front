...
function Singin(props) {
  ...
  const handleOnSubmit = async (e) => {
    e.preventDefault();
    const body = { ...usuario, rol: "USER_ROLE" };
    ...
    if (res && documento) {
      ...
      file.append("documento", documento[0]);
       const doc = await actualizarImagen(file, res.uid);
    ...
    if (!res.msg) {
      const body = { correo: usuario.correo, contraseña: usuario.contraseña };

      let acceso = await login(body);
        ...
        localStorage.setItem("Usuario", acceso.usuario.uid);
      ...
      <form className="formulario">
        ...
        <div className="contenedor">
          ...
          {validaciones.nombre === false && (
            <p className="mensaje"> Se requiere nombre y apellido</p>
          )}
          <Input
            name="correo"
            estadoValidacion={validaciones}
          ...
          {validaciones.correo === false && (
            <p className="mensaje">
              El correo debe tener el sig formato example@hotmail.com
            </p>
          )}
          <Input
            estadoValidacion={validaciones}
          ...
          {validaciones.contraseña === false && (
            <p className="mensaje">Debe contener al menos 8 caracteres </p>
          )}
          <Input
            name="img"
            type="file"
